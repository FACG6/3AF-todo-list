...
const todos1 = [{ id: -1, description: ['hgg', 4], done: false }, { id: 0, description: ['fatma', 5], done: false }];
const newtodo = ['ahfgfg', 6];
...
test('Add -todoarray', function (t) {
  ...
  const expected = [{ id: -1, description: ['hgg', 4], done: false },
  { id: 0, description: ['fatma', 5], done: false },
  { id: 1, description: ["angham", 0], done: false }
  ];
...
test('Sort -todoarray', function (t) {
  ...
  const expected = [
    { id: -1, description: ['hgg', 4], done: false },
    { id: 0, description: ['fatma', 5], done: false }];
  t.deepEqual(actual, expected, 'Sorted Test ');
  t.end();
});
const todos = [
...
test('testing markTodo function', function (t) {
  ...
  t.deepEqual(actual, expected, 'the actual should be equal expected 123');
  t.end();
});
